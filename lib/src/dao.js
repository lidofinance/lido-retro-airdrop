...
async function getFinance(web3, address) {
  Finance.setProvider(web3.currentProvider)
  return await Finance.at(address)
}
...
async function proposePayment(voting, tokenManager, finance, payment, txOpts) {
  const { tokenAddress, recipient, amount, reference } = payment
  const calldata = await finance.contract.methods.newImmediatePayment(
    tokenAddress,
    recipient,
    amount,
    reference
  ).encodeABI()
  const evmScript = encodeCallScript([{ to: finance.address, calldata }])
  const voteDesc = `Send ${amount} tokens at ${tokenAddress} to ${recipient}: ${reference}`
  return await createVote(voting, tokenManager, voteDesc, evmScript, txOpts)
}
...
async function castVote(voting, voteOpts, txOpts) {
  const { voteId, supports, executesIfDecided } = voteOpts
  return await voting.vote(voteId, supports, executesIfDecided, txOpts)
}
